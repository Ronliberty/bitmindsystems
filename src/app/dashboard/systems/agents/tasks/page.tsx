  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        const filtered = data.filter((t: Task) => t.task_type === "agent");
        setTasks(filtered);
      } catch (e) {
    ...
    <section>
      ...
      >
        ...
        <p className="text-gray-400 mt-1">
          Tasks assigned to you or available for agent work.
        </p>
      </motion.div>
      ...
      {!loading && tasks.length === 0 && (
        <p className="text-gray-400 text-center py-10">
          No agent tasks available right now.
        </p>
      )}
      ...
      <div className="space-y-4">
        {tasks.map((task) => (
          ...
          >
            ...
            {/* Task Details from JSON */}
            {task.task_details && (
              <div className="mb-3 text-sm text-gray-400">
                <p>
                  <span className="font-medium">Client ID:</span>{" "}
                  {task.task_details.client_id || "N/A"}
                </p>
                <p>
                  <span className="font-medium">Region:</span>{" "}
                  {task.task_details.region || "N/A"}
                </p>
                <p>
                  <span className="font-medium">Commission Rate:</span>{" "}
                  {task.task_details.commission_rate || "N/A"}
                </p>
              </div>
            )}
